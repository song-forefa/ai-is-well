...
// 썸네일이 없는 항목도 그리드에서 비어 보이지 않도록,
// id 로부터 항상 같은 색이 나오는 그라디언트를 만들어 준다.
...
// 카드/히어로의 이미지 자리에 넣을 글리프.
// 이모지 필드 > 제목에 들어 있는 이모지 > 유형 기본 아이콘.
export function placeholderGlyph(item: Item): string {
